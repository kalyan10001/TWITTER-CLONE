...
export const createPost=async(req,res)=>{
    try {
        ...
        if(!user && !img){
            return res.status(200).json({error:"post must have a text or img"});
        }
...
export const deletePost=async(req,res)=>{
    try {
        ...
        if(post.user.toString()!==req.user._id.toString()){
            return res.status(200).json({error:"you are not authorized to delete this post"});
        }
...
export const commentOnPost=async(req,res)=>{
    try {
        ...
        if(!post){
            return res.status(200).json({error:"post is not found"});
        }
...
export const likeUnlikePost=async(req,res)=>{
    try {
        ...
        res.status(200).json({error:"internal server error"});
        console.log("error in like/unlike post controller");   
    }
};
